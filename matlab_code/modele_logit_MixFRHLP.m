function [probas, loglik] = modele_logit_MixFRHLP(W,M,Y,Gamma)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%   
% Cette fonction peut egalement ï¿½tre utilisï¿½e pour calculer seulement les 
% probas de la faï¿½oc suivante : probas = modele_logit(W,M)
%
% Faicel Chamroukhi 31 Octobre 2008 (mise à jour)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if nargin > 2
    [n1,K] = size(Y);
...
if nargin>2    %calcul de la log-vraisemblance
    loglik = sum(sum((Gamma.*(Y.*MW)) - ((Gamma.*Y).*log(sum(expMW,2)*ones(1,K))),2));
%     % sans les hig
%     loglik = sum(hig*loglik);
     if isnan(loglik)
        ...
        loglik = sum(sum(((Gamma.*Y).*MW) - ((Gamma.*Y).*log(sum(expMW,2)*ones(1,K)+eps)),2));        
    end
    if isnan(loglik)
